...
export function IndexRoot(): JSX.Element {
	...
	const updateHex = useCallback((newHex: string) => (
		setColor((oldState) => oldState.hex === newHex ? oldState : createColorState(newHex))
	), [setColor]);
	...
	return (
		<IndexContext.Provider value={ctx}>
			<ScopedCssBaseline sx={RootStyles} component="div" className="viewRoot">
...
const createColorState = (hex: string): ColorState => ({ argb: ColorTools.ArgbFromHex(hex), hex: hex });
